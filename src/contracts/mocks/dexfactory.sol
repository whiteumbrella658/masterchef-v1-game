...
contract PancakeswapERC20 is IPancakeswapERC20 {
    ...
    string public constant override name = 'Pancakeswap ';
    string public constant override symbol = 'Pancake-LP';
    ...
    function permit(address _owner, address _spender, uint _value, uint deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(deadline >= block.timestamp, 'Pancakeswap: EXPIRED');
        bytes32 digest = keccak256(
            abi.encodePacked(
                '\x19\x01',
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(PERMIT_TYPEHASH, _owner, _spender, _value, nonces[_owner]++, deadline))
            )
        );
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress != address(0) && recoveredAddress == _owner, 'Pancakeswap: INVALID_SIGNATURE');
