...
contract MasterChef is Ownable{
    ...
    uint public presaleStart = 1649098800;
    uint public presaleEnd = 1649185200;
    ...
    constructor(address _token, address _team, address _backup, address _designer) {
        token = IMSCH(_token);
        team = _team;
        backup = _backup;
        designer = _designer;

        IPancakeswapRouter _pancakeswapRouter = IPancakeswapRouter(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);                 // To be changed
    ...
    function addLiquidityFirst() public payable{
        require(msg.value >= 1*10**18, "Insufficient funds for adding liquidity");                  // To be changed
        address[] memory path = new address[](2);
        path[0] = address(weth);
        path[1] = address(token);
   
        token.mint(address(this), 17*10**17);
        token.approve(address(router), 17*10**17);
    ...
    function createChef(address _charity) public payable {
        ...
        require(
            ...
            msg.value >= 3 * 10**18,                    // To be changed
    ...
    function getHarvestSeconds(Recipe _food) private view returns (uint) {
        if (_food == Recipe.Crepe) {
            ...
            if(kettleNFTs == 0){
                ...
                return 4 * 60 * 60 / 3;             // 1 hour 20 min for Kettle owners
            }
    ...
    function getLandPrice(uint landSize) private view returns (uint price) {
        ...
        if (landSize == 5) {
            ...
            return 50 * 10**decimals;       //to be changed
        } else if (landSize == 14) {
            return 100 * 10**decimals;      //to be changed
        }
    ...
    function levelUp() public hasChef {
        ...
        TransferHelper.safeTransfer(address(token), address(backup), fmcPrice.div(10));              // %20 will be transferred to backup contract
        TransferHelper.safeTransfer(address(token), address(team), fmcPrice.mul(3).div(10));                 // %20 will be transferred to team
    ...
    function setPresalePeriod(uint _presaleStart, uint _presaleEnd) public onlyOwner {
        presaleStart = _presaleStart;
        presaleEnd = _presaleEnd;
    }

    function withdrawMigration() public onlyOwner {
        address payable payTeam = payable(team);
        payTeam.transfer(address(this).balance);
        TransferHelper.safeTransfer(address(token), address(team), token.balanceOf(address(this)));
    }

    function mintNFT(NFT _symbol) public payable{
        uint256 price;
        require(fridge != address(0), "Fridge NFT address is not set yet");
        require(ladle != address(0), "Ladle NFT address is not set yet");
        ...
        if(_symbol == NFT.Fridge){
            if(block.timestamp > presaleStart && block.timestamp < presaleEnd){
                price = 125 * 10 ** 18;
            }else{
                price = 250 * 10 ** 18;
            }
            require(msg.value >= price, "Insufficient funds for Fridge");               // to be changed
            ...
            maticRewardsOpendAt[msg.sender] = block.timestamp;
        }else if(_symbol == NFT.Ladle){
            if(block.timestamp > presaleStart && block.timestamp < presaleEnd){
                price = 100 * 10 ** 18;
            }else{
                price = 200 * 10 ** 18;
            }
            require(msg.value >= price, "Insufficient funds for Ladle");               // to be changed
            
            ERCItem(ladle).mint(msg.sender);
            mschRewardsOpendAt[msg.sender] = block.timestamp;

        }else if(_symbol == NFT.Kettle){
            price = 100 * 10 ** 18;                 // To be changed
            require(msg.value >= price, "Insufficient funds for Kettle");               // to be changed

            ERCItem(kettle).mint(msg.sender);
        }else if(_symbol == NFT.Mixer){
            price = 100 * 10 ** 18;                 // To be changed
            require(msg.value >= price, "Insufficient funds for Mixer");               // to be changed

            ERCItem(mixer).mint(msg.sender);            
        }else if(_symbol == NFT.Oven){
            price = 100 * 10 ** 18;                 // To be changed
            require(msg.value >= price, "Insufficient funds for Oven");               // to be changed
